<template>
    <div class="w-full rounded-md shadow-md dark:bg-dark-primary dark:text-white bg-white flex justify-center flex-col gap-2 py-1" v-if="post">
        ...
        <div class="px-2">
            <i class="bx bx-like"></i> {{ post.likes.length ?? 0}}
            <i class="bx bx-chat"></i> {{ post.comments ?? 0}}
        </div>
...
const emit = defineEmits(["selectPost", "deletePost", "editPost", "likePost", "sharePost", "bookmarkPost", "deleteBookmarkPost"]);
